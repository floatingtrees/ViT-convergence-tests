...
checkpoint = torch.load("model_and_optimizer_opaque50.pth")
...
for epoch in range(num_epochs):
    total_past_steps = math.ceil(epoch * len(dataset) / batch_size)
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(validation_dataloader):
            opacity = opacity_scheduler.sample((total_past_steps) / num_steps)
            x, y = batch
            x = x.to(device)
            y = y.to(device)
    ...
    for j, batch in enumerate(dataloader):
        ...
        opacity = opacity_scheduler.sample((total_past_steps + j) / num_steps)
        with open("opacities.txt", 'a') as file:
            file.write(f'Step {total_past_steps + j} out of {num_steps} in epoch {epoch}, opacity: {opacity}\n')
        x, y = batch
        x = x.to(device)
        y = y.to(device)
        ...
        if j % 500 == 499:
            ...
            sys.stdout.flush()
    print(optimizer.param_groups[0]['lr'])
    save_path = f"model_and_optimizer_opaque{epoch}.pth"
    torch.save({
